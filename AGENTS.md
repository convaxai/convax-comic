...
13. **改完必须验证，失败不得交付。** 每次代码、配置、依赖或文档关联行为改动后，
   必须先运行受影响范围的定向测试，再运行根 `yarn check`；涉及 profile、运行时、
   桌面启动或打包链路时，还必须串行运行相应的 `yarn smoke:upstream` 或
   `yarn package:dir`。不得把会清理/重建同一产物的命令并发执行。任何验证失败都要
   先修复并重新验证；无法通过时必须明确报告阻塞和失败日志，不能声称改动已完成。
